...
SERVICE_TASK_REVISION=$(aws ecs register-task-definition --family ${TASK_FAMILY_SERVICE} --container-definitions ${SERVICE_CONTAINER_DEFINITIONS} --requires-compatibilities EC2 | php -r "echo json_decode(file_get_contents('php://stdin'))->taskDefinition->revision;")
...
aws ecs register-task-definition --family ${TASK_FAMILY_TASK} --container-definitions ${TASK_CONTAINER_DEFINITIONS} --requires-compatibilities EC2  > /dev/null
